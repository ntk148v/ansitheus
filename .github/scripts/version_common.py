...
def fetch_latest_github_release(repo: str, token: str) -> Optional[str]:
    ...
    try:
        ...
        print(
            f"::warning::Failed to pull release info for {repo}: {exc}", file=sys.stderr
        )
        return None
    except urllib.error.URLError as exc:  # pragma: no cover - network failure
        print(
            f"::warning::Network issue while calling GitHub for {repo}: {exc}",
            file=sys.stderr,
        )
        return None
    ...
    if not tag_name:
        print(
            f"::warning::Release payload for {repo} missing tag_name", file=sys.stderr
        )
        return None
    ...
    for idx, line in enumerate(lines):
        ...
        if inside_block and stripped.startswith("default:"):
            ...
            if (
                ...
                and remainder.endswith(remainder[0])
                and len(remainder) > 1
            ):
                quote = remainder[0]
